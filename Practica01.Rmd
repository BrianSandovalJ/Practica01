...
3/. Analizar los distintostiposdepeticionesHTTP(GET,POST,PUT,DELETE) gestionadas por el servidor, identificando la frecuencia de cada una de estas. Repetir el análisis, esta vez filtrando previamente aquellas peticiones correspondientes a recursos ofrecidos de tipo imagen.
...
4/. Generar al menos 2 gráficos distintos que permitan visualizar alguna característica relevante de los datos analizados.

- Mostar los 4 origenes con mas bytes generados

```{r , echo=F}
soloIP <- grep("^(1|2|3)", tablaFlot4$Origen)
datos_IP <- tablaFlot4[soloIP, ]
datos_IP <- as.data.frame(datos_IP)

datos_ordenados <- datos_IP[order(-datos_IP$Bytes), ]

maximos <- datos_ordenados[1:4, c("Origen", "Bytes", "URL")]

htmlTable(maximos)
```

- Cantidad de bytes con archivos .zip

```{r , echo=F}
soloZIP <- grep(".zip$", tablaFlot4$URL)
datos_ZIP <- tablaFlot4[soloZIP, ]
datos_ZIP <- as.data.frame(datos_ZIP)

datos_ordenados <- datos_ZIP[order(-datos_ZIP$Bytes), ]

ggplot(datos_ordenados, aes(x = reorder(URL, +Bytes), y = Bytes)) +
  geom_bar(stat = "identity", fill = "lightblue") +
  coord_flip() +
  labs(title = "Gráfico de Barras Ordenado de Bytes", x = "DOC", y = "Bytes") +
...
```{r , echo=F}
ggplot(tablaFlot4, aes(x = tablaFlot4$Dia, y = tablaFlot4$Peti)) +
  geom_line(color = "blue") +
  geom_point(color = "red") +
  labs(title = "Número de Peticiones Servidas a lo Largo del Tiempo", x = "Fecha", y = "Peticiones") +
  theme_minimal()
```
...
perform_kmeans <- function(k) {
  ...
  data_onehot$cluster <- as.factor(kmeans_results$cluster)

  ggplot2::ggplot(data_onehot, aes(x = resourceLenght, Bytes, colour = cluster)) +
