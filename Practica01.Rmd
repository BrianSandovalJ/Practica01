...
El data frame tiene `r nrow(epa_http)` filas y `r ncol(epa_http)` columnas.
...
El valos medio es: `r tabla_flot <- epa_http
colnames(tabla_flot) <- c("Origen","Tiempo","Tipo Peticion", "URL", "Version","Respuesta","Bytes")
suppressWarnings(bytes<- as.numeric(tabla_flot$Bytes))
suppressWarnings(bytes[is.na(bytes)] <- 0)
suppressWarnings(total_bytes <- mean(bytes))
total_bytes
`

```{r pregunta02, echo=FALSE}
#dim(epa_http)
...
#COD 400:tu navegador envía una solicitud al servidor del sitio web y el servidor no puede procesar o reconocer la solicitud
#COD 501: el servidor no soporta la funcionalidad necesaria para satisfacer la solicitud
print("Valor medio de Bytes")
tabla_flot <- epa_http
colnames(tabla_flot) <- c("Origen","Tiempo","Tipo Peticion", "URL", "Version","Respuesta","Bytes")
#tabla_flot$Bytes[tabla_flot$Bytes == "-" & tabla_flot$Respuesta == "404"] <- 0
#tabla_flot$Bytes[tabla_flot$Bytes == "-" & tabla_flot$Respuesta == "302"] <- 1
#tabla_flot$Bytes[tabla_flot$Bytes == "-" & tabla_flot$Respuesta == "400"] <- 2
#tabla_flot$Bytes[tabla_flot$Bytes == "-" & tabla_flot$Respuesta == "403"] <- 2
#tabla_flot$Bytes[tabla_flot$Bytes == "-" & tabla_flot$Respuesta == "501"] <- 100
#tabla_flot[is.na(tabla_flot)] <- 0
suppressWarnings(bytes<- as.numeric(tabla_flot$Bytes))
suppressWarnings(bytes[is.na(bytes)] <- 0)
suppressWarnings(total_bytes <- mean(bytes))
...
```{r , echo=FALSE}
tablaflot3 <- epa_http
informacion_GET <- tablaflot3[grepl("GET", tablaflot3$`Tipo Peticion`, ignore.case = FALSE), ]
dia_hora <- substr(informacion_GET$Tiempo,5,6)
...
#length(unique(epa_http$Origen[grepl(".edu", epa_http$Origen)]))
tablaflot2 <- epa_http
#tablaflot2$Bytes[tablaflot2$Bytes == "-" & tablaflot2$Respuesta == "404"] <- 0
#tablaflot2$Bytes[tablaflot2$Bytes == "-" & tablaflot2$Respuesta == "302"] <- 1
#tablaflot2$Bytes[tablaflot2$Bytes == "-" & tablaflot2$Respuesta == "400"] <- 2
#tablaflot2$Bytes[tablaflot2$Bytes == "-" & tablaflot2$Respuesta == "403"] <- 2
#tablaflot2$Bytes[tablaflot2$Bytes == "-" & tablaflot2$Respuesta == "501"] <- 100
...
#suppressWarnings(bytes<- as.numeric(descargas_txt$Bytes))
suppressWarnings(bytes<- as.numeric(descargas_txt$Bytes))
suppressWarnings(bytes[is.na(bytes)])
suppressWarnings(suma <- mean(bytes))
suppressWarnings(numero_sum <- sum(suma))

suppressWarnings(print(numero_sum))
```
